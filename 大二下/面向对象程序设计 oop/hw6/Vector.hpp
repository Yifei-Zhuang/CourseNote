...
class Vector {
 public:
  ...
  T &at(int index);  // accesses the specified element, throws an exception of type 'std::out_of_range' when index <0 or
                     // >=m_nSize
 ...
 private:
  ...
  int m_nCapacity{20};  // the number of elements that can be held in currently allocated storage
};
